...
def user_input():
    try:
        id, pw = map(str, input("아이디와 비밀번호를 차례대로 입력해주세요 : ").split(','))
        return id, pw
    except:
        print("올바르지 않은 입력입니다!")
        id, pw = user_input()
...
def signin(id, pw):
    f = open('students_info.csv', 'r', encoding='utf-8')
    flag = 0
...
def signup(dict_user) :
    f = open('students_info.csv', 'a', encoding='utf-8', newline='')
    wr = csv.writer(f)
    if usercheck(dict_user['uid']):
        wr.writerow([dict_user['uid'],"",dict_user['password']])
        userlist()
    f.close()
